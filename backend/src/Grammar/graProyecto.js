...
break;
case 127:
this.$ = new Coantinue( $$[$0-1], this._$.first_line, this._$.first_column);
break;
case 128:
...
case 2:return 88 
break;
case 3:console.log("STRING QUEMADO ");return 85
break;
...
case 15:return 72
break;
case 16:return 60 // punto pero TOKEN
break;
case 17:return 77
break;
case 18:return 76 ;
...
case 59:return 5
break;
case 60: CErrores.Errores.add(new CNodoError.NodoError("LEXICO","No se esperaba el caracter: "+yy_.yytext,yy_.yylloc.first_line));  console.error('Este es un error léxico: ' + yy_.yytext + '  en la linea: ' + yy_.yylloc.first_line + ', en la columna: ' + yy_.yylloc.first_column);
break;
}
},
rules: [/^(?:\s+[ \t\r\n\f]\n\/\/.*[\/][*][^*]*[*]+([^\/*][^*]*[*]+)*[\/](('[^☼]')))/,/^(?:(([0-9]+)(\.([0-9]+))?))/,/^(?:{entero})/,/^(?:(("[^"]*")))/,/^(?:{comentarioBloque})/,/^(?:{comentarioLinea})/,/^(?::)/,/^(?:\/)/,/^(?:;)/,/^(?:--)/,/^(?:-)/,/^(?:\+\+)/,/^(?:\+)/,/^(?:\*)/,/^(?:\^)/,/^(?:%)/,/^(?:\.)/,/^(?:<=)/,/^(?:>=)/,/^(?:<)/,/^(?:>)/,/^(?:==)/,/^(?:!=)/,/^(?:\|\|)/,/^(?:&&)/,/^(?:!)/,/^(?:=)/,/^(?:,)/,/^(?:\()/,/^(?:\))/,/^(?:\{)/,/^(?:\})/,/^(?:main\b)/,/^(?:println\b)/,/^(?:print\b)/,/^(?:out\b)/,/^(?:System\b)/,/^(?:void\b)/,/^(?:return\b)/,/^(?:if\b)/,/^(?:else\b)/,/^(?:switch\b)/,/^(?:case\b)/,/^(?:default\b)/,/^(?:break\b)/,/^(?:continue\b)/,/^(?:while\b)/,/^(?:do\b)/,/^(?:for\b)/,/^(?:false\b)/,/^(?:true\b)/,/^(?:class\b)/,/^(?:import\b)/,/^(?:char\b)/,/^(?:double\b)/,/^(?:int\b)/,/^(?:String\b)/,/^(?:boolean\b)/,/^(?:{id})/,/^(?:$)/,/^(?:.)/],
