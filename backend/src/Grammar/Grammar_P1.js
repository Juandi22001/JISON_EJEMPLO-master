...
case 57:return 12
break;
case 58:return 5          // ESTE TOKEN ES MUY IMPORTANTE ES EL DE FINALIZACION
break;
case 59:console.error('ERROR LEXICO: ' + yy_.yytext + '  en la linea: ' + yy_.yylloc.first_line + ', en la columna: ' + yy_.yylloc.first_column);
break;
}
},
rules: [/^(?:\s+)/,/^(?:\/\/.*[/][*][^*]*[*]+([^/*][^*]*[*]+)*[/]([0-9]+))/,/^(?:(([0-9]+)(\.([0-9]+))?))/,/^(?:(('[^☼]')))/,/^(?:(("[^"]*")))/,/^(?:\*)/,/^(?:\/)/,/^(?:\.)/,/^(?:;)/,/^(?:--)/,/^(?:-)/,/^(?:\+\+)/,/^(?:\+)/,/^(?:\*)/,/^(?:<)/,/^(?:>)/,/^(?:<=)/,/^(?:>=)/,/^(?:==)/,/^(?:!=)/,/^(?:\|\|)/,/^(?:&&)/,/^(?:!)/,/^(?:=)/,/^(?:\()/,/^(?:\))/,/^(?:\{)/,/^(?:\})/,/^(?:\[)/,/^(?:\])/,/^(?:true\b)/,/^(?:false\b)/,/^(?:Console\b)/,/^(?:if\b)/,/^(?:else\b)/,/^(?:break\b)/,/^(?:continue\b)/,/^(?:while\b)/,/^(?:bool\b)/,/^(?:float\b)/,/^(?:int\b)/,/^(?:for\b)/,/^(?:char\b)/,/^(?:string\b)/,/^(?:void\b)/,/^(?:public\b)/,/^(?:default\b)/,/^(?:private\b)/,/^(?:main\b)/,/^(?:switch\b)/,/^(?:case\b)/,/^(?:break\b)/,/^(?:return\b)/,/^(?:continue\b)/,/^(?:Console\b)/,/^(?:write\b)/,/^(?:do\b)/,/^(?:(([a-zA-Z_])[a-zA-Z0-9_]*))/,/^(?:$)/,/^(?:.)/],
